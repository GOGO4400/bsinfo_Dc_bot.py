...
async def send_complaint_to_channel(server_name, time, myself, ign, useracid, fign, acid, linkedaccount, offender, complaint):
    ...
    embed = discord.Embed(title=f"Complaint on {server_name}", color=0xE74C3C)
    #embed.add_field(name="Complaint Time", value=time, inline=False)
    embed.add_field(name="Complainant (IGN / Account ID)", value=f"{myself} ({ign}) / {useracid}", inline=False)
    embed.add_field(name="Offender (IGN / Account ID)", value=f"{offender} ({fign}) / {acid}", inline=False)
    embed.add_field(name="Linked Accounts of Offender", value=linkedaccount if linkedaccount else "None", inline=False)
    embed.add_field(name="Type of Complaint", value=complaint, inline=False)
    embed.set_footer(text="Please handle this complaint promptly.")
    ...
    # Ping role by sending plain text with role name (assuming role is a string with format <@&ID>)
    await channel.send(f"{complaint_role}")
'''

#simple complain formet 
async def send_complaint_to_channel(server_name, time, myself, ign, useracid, fign, acid, linkedaccount, offender, complaint):
...
#embed msg with dc user id mention formet
...
async def send_complaint_to_channel(server_name, time, myself, ign, useracid, fign, acid, linkedaccount, offender, complaint):
    ...
    try:
        sender_link = mongo.linkedusers.find_one({"pbid": useracid})
        if sender_link and "discord_id" in sender_link:
            linkedaccount_sender = f"<@{sender_link['discord_id']}>"
        else:
            linkedaccount_sender = "None"
    except:
        linkedaccount_sender = "None"

    try:
        offender_link = mongo.linkedusers.find_one({"pbid": acid})
        if offender_link and "discord_id" in offender_link:
            linkedaccount_offender = f"<@{offender_link['discord_id']}>"
        else:
            linkedaccount_offender = "None"
    except:
        linkedaccount_offender = "None"

    embed = discord.Embed(title=f"Complaint on {server_name}", color=0xE74C3C)
    embed.add_field(name="Complainant", value=f"{myself} ({ign}) / {useracid} {linkedaccount_sender}", inline=False)
    embed.add_field(name="Offender", value=f"{offender} ({fign}) / {acid} {linkedaccount_offender}", inline=False)
    embed.add_field(name="Complaint", value=complaint, inline=False)
    embed.set_footer(text=f"{complaint_role}")
...
async def joined_player(pbid, devices_string, time):
    ...
    message = f"{devices_string} [{pbid}] has joined {serverss}. Linked accounts: {otheraccounts}. {notify_role}"
    await channel.send(message)
'''


#replace function vale character
'''
async def joined_player(pbid, devices_string, time):
    channel_id = notifyChannelID
    channel = client.get_channel(channel_id)
    ...
    # Replace specific characters in devices_string and otheraccounts
    def replace_chars(text):
        text = text.replace("", "<:cr:1271120318861283399>")
        text = text.replace("", "<:bs_pheonix:1252440405115408517>")
        text = text.replace("", "<:skull_bs:1252440222319247452>")
        text = text.replace("", "<:bs_v2account:1251940770378682429>")
        text = text.replace("", "<:de:1271120315056918579>")
        text = text.replace("", "<:bs_pheonix:1252440405115408517>")
        return text

    devices_string = replace_chars(devices_string)
    otheraccounts = replace_chars(otheraccounts)
...
async def joined_player(pbid, devices_string, time):
    ...
    # Replace special characters in the entire message
    message = message.replace("", "<:cr:1271120318861283399>")
    message = message.replace("", "<:bs_pheonix:1252440405115408517>")
    message = message.replace("", "<:skull_bs:1252440222319247452>")
    message = message.replace("", "<:bs_v2account:1251940770378682429>")
    message = message.replace("", "<:de:1271120315056918579>")
    message = message.replace("", "<:bs_pheonix:1252440405115408517>")
...
async def on_message(message):
    ...
    if message.channel.id == logsChannelID:
        # Format the message content with sender's name
        chat_message = f"[Discord] {message.author.name}: {message.content}"
        _ba.pushcall(Call(ba.internal.chatmessage, chat_message), from_other_thread=True)
                ...
                if len(m) == 2:
                    try:
                        ...
                        if mp > 0:
                            ...
                            # Thread-safe call to game chat
                            _ba.pushcall(Call(ba.internal.chatmessage, f"Max player limit set to {mp} from Discord"), from_other_thread=True)
                        else:
                            await message.channel.send(f">>> Error!!: Enter a valid positive number")
                    except ValueError:
                        await message.channel.send(f">>> Error!!: Enter a valid number")
                else:
                    await message.channel.send(f">>> Invalid format! Use `{prefix}{commands_prefix}mp <count>`")
           '''         
            ...
            ''' 
                ...
                if len(m) == 2:
                    ...
                    if serverdata.recents:
                        ...
                        # Game me bhi message send karna, thread-safe call ke saath
                        _ba.pushcall(Call(ba.internal.chatmessage, f"kicked {pbid} though dc"), from_other_thread=True)
                    else:
                        await message.channel.send(f">>> `{pbid}` not joined the server yet..!")
                else:
                    await message.channel.send(f">>> Invalid Format! Use `{prefix}{commands_prefix}kick <pbid>`")
            '''
...
### tag and effect add cmd in specific server and all server :)))))
        ...
        '''    elif m[0] == (cmd + 'top10'):
                ...
                if stats:
                        ...
                        embed = discord.Embed(
                            title=f"Top 10 Players of {servers}",
                            description="",
                        ...
                        for rank, (player_id, player_data) in enumerate(top_10_players, start=1):
                            ...
                            # Character replacements (aapki given replaces)
                            player_name = player_name.replace("", "<:cr:1271120318861283399>")
                            player_name = player_name.replace("", "<:bs_pheonix:1252440405115408517>")
                            player_name = player_name.replace("", "<:skull_bs:1252440222319247452>")
                            player_name = player_name.replace("", "<:bs_v2account:1251940770378682429>")
                            player_name = player_name.replace("", "<:de:1271120315056918579>")
                            player_name = player_name.replace("", "<:bs_pheonix:1252440405115408517>")
                            player_name = player_name.replace("", "<:bs_logo:1252440615011090452>")
                            ...
                            embed.add_field(
                                ...
                                value=f"Account-ID: {player_accountid}, Score: {player_scores}, KD-Score: {player_kd}",
                                inline=False
                            )
...
async def refresh_stats():
    ...
    while not client.is_closed():
        ...
        try:
            ...
            print(f"ERROR OCCURRED IN BOT.PY:\n{err}")

        # Replace characters in msg
        message = msg.replace("", "<:cr:1271120318861283399>")
        message = message.replace("", "<:bs_pheonix:1252440405115408517>")
        message = message.replace("", "<:skull_bs:1252440222319247452>")
        message = message.replace("", "<:bs_v2account:1251940770378682429>")
        message = message.replace("", "<:de:1271120315056918579>")
        ...
        embed = discord.Embed(
            ...
            color=0x00FF00  # Replace with your desired color (in hex)
        )
        embed.set_author(name="LIVE SERVER STATUS | NODE #1", icon_url="https://cdn.discordapp.com/emojis/878301194865508422.gif?size=512")
       # embed.add_field(name=" **Server's Info**", value=f"py\nIP = {ip} \nPORT = {port}", inline=False)    
        embed.add_field(name="<:squad:1252440171400532060> **Players**", value=f"\n{len(stats['roster'])}/{ba.internal.get_public_party_max_size()} ```
        embed.add_field(name="<:emoji_43:1292804927634276352> **PLAYERS IN SERVER:**", value=f" \n{message if len(message) != 0 else 'I am alone.. :('}`` ", inline=False)
        embed.add_field(name="<:emoji_45:1292805041782263828> **Server's Info**", value=f"```\nIP = {ip} \nPORT = {port}```
        embed.set_footer(text="Auto updates every 10 seconds!", icon_url='https://cdn.discordapp.com/emojis/842886491533213717.gif?size=96&quality=lossless')
        
        await livestatsmsgs.edit(embed=embed)
        chat_embed = await get_chats()

        if chat_embed.description != 'disabled':  # Only update the live chat if it's not disabled
            await livestatsmsgs.edit(embed=chat_embed)        
        await asyncio.sleep(10)
        '''
